...
import { Shield, Fingerprint, Smartphone, Key, Settings, AlertTriangle, CheckCircle } from 'lucide-react';
...
const MultiModalAuth = ({ onAuthSuccess, onAuthFailure, mode = 'login' }) => {
  ...
  const handleLogin = async () => {
    ...
    const result = await webauthnService.authenticate(formData.email, selectedMethod);
    
    if (result.success) {
      setSuccess('Authentication successful!');
      if (result.usedFallback) {
        setSuccess(`Authentication successful using fallback method: ${result.fallbackMethod}`);
      }
  ...
  const handleRegistration = async () => {
    ...
    if (result.success) {
      ...
      if (result.usedFallback) {
        setSuccess(`Registration successful using fallback method: ${result.fallbackMethod}`);
      }
      onAuthSuccess?.(result.data);
    } else {
    ...
    return (
      <div className="card" style={{ background: '#fff3cd', border: '1px solid #ffeaa7' }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span style={{ fontWeight: '600', color: '#856404' }}>WebAuthn Not Supported</span>
        </div>
        <p style={{ fontSize: '14px', color: '#856404' }}>
          Your browser doesn't support WebAuthn. Please use a modern browser like Chrome, Firefox, or Edge.
        </p>
      </div>
    );
  }
    ...
    <div className="card">
      <div className="text-center mb-4">
        ...
        <p className="text-muted">
          {mode === 'login' ? 'Sign in with your preferred method' : 'Set up secure authentication'}
        </p>
      </div>
      ...
      <div className="card" style={{ background: '#2a2a2a', marginBottom: '16px' }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span style={{ fontWeight: '500', color: '#007bff' }}>Device Capabilities</span>
        </div>
          ...
          {capabilities.availableMethods.map((method) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div style={{ fontWeight: '500' }}>{getMethodLabel(method)}</div>
      ...
      {success && (
        <div className="card" style={{ background: '#d4edda', border: '1px solid #c3e6cb', marginTop: '16px' }}>
          <div className="flex items-center gap-2">
      ...
      {error && (
        <div className="card" style={{ background: '#f8d7da', border: '1px solid #f5c6cb', marginTop: '16px' }}>
          <div className="flex items-center gap-2">
      ...
      {/* Method Information */}
      {selectedMethod && (
        <div className="card" style={{ background: '#2a2a2a', marginTop: '16px' }}>
          <div className="flex items-center gap-2 mb-2">
            <Shield size={16} color="#28a745" />
            <span style={{ fontWeight: '500', color: '#28a745' }}>Selected Method</span>
          </div>
          <div className="text-sm">
            <div className="flex items-center gap-2 mb-1">
              {getMethodIcon(selectedMethod)}
              <span style={{ fontWeight: '500' }}>{getMethodLabel(selectedMethod)}</span>
            </div>
