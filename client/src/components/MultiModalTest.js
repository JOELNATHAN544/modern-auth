...
import { Shield, Settings, AlertTriangle, CheckCircle, Play, Square, RotateCcw } from 'lucide-react';
...
const MultiModalTest = () => {
  ...
  const runAllTests = async () => {
    ...
    addLog('info', 'Starting comprehensive multi-modal authentication tests...');
  ...
  const testAuthenticationMethods = async () => {
    ...
    for (const method of methods) {
      try {
        ...
        const testResult = await webauthnService.testAuthenticationMethod(method);
    ...
    for (const method of methods) {
      try {
        ...
        results[method] = {
          ...
          fallbackMethod,
          fallbackAvailable: fallbackMethod ? capabilities?.availableMethods.includes(fallbackMethod) : false
        };
  ...
  const testCrossPlatformCompatibility = async () => {
    ...
    const results = {
      ...
      mixedModeSupported: capabilities?.availableMethods.includes('both') || false,
      userVerificationRequired: true
    };
    
    setTestResults(prev => ({ ...prev, crossPlatformCompatibility: results }));
    ...
    <div className="container">
      <div className="card">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 style={{ fontSize: '24px', fontWeight: '600' }}>Multi-Modal Authentication Test Suite</h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            {testResults.deviceCapabilities && (
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span style={{ color: getStatusColor(testResults.deviceCapabilities.webauthnSupported ? 'passed' : 'failed') }}>
                    {testResults.deviceCapabilities.webauthnSupported ? '✅' : '❌'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Platform Auth:</span>
                  <span style={{ color: getStatusColor(testResults.deviceCapabilities.platformAuthenticator ? 'passed' : 'failed') }}>
                    {testResults.deviceCapabilities.platformAuthenticator ? '✅' : '❌'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Secure Context:</span>
                  <span style={{ color: getStatusColor(testResults.deviceCapabilities.secureContext ? 'passed' : 'failed') }}>
                    {testResults.deviceCapabilities.secureContext ? '✅' : '❌'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Available Methods:</span>
                  <span>{testResults.deviceCapabilities.availableMethods?.length || 0}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="card" style={{ background: '#2a2a2a' }}>
            ...
            {testResults.authenticationMethods && (
              <div className="space-y-2 text-sm">
                {Object.entries(testResults.authenticationMethods).map(([method, result]) => (
                  <div key={method} className="flex justify-between">
                    <span style={{ textTransform: 'capitalize' }}>{method}:</span>
                    <span style={{ color: getStatusColor(result.available ? 'passed' : 'failed') }}>
                      {result.available ? '✅' : '❌'}
                    </span>
                  </div>
                ))}
          ...
          <div className="card" style={{ background: '#2a2a2a' }}>
            ...
            {testResults.fallbackMechanisms && (
              <div className="space-y-2 text-sm">
                {Object.entries(testResults.fallbackMechanisms).map(([method, result]) => (
                  <div key={method} className="flex justify-between">
                    <span style={{ textTransform: 'capitalize' }}>{method}:</span>
                    <span style={{ color: getStatusColor(result.hasFallback ? 'passed' : 'failed') }}>
          ...
          <div className="card" style={{ background: '#2a2a2a' }}>
            ...
            {testResults.crossPlatformCompatibility && (
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Platform Auth:</span>
                  <span style={{ color: getStatusColor(testResults.crossPlatformCompatibility.platformAuthenticator ? 'passed' : 'failed') }}>
                    {testResults.crossPlatformCompatibility.platformAuthenticator ? '✅' : '❌'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Cross-Platform:</span>
                  <span style={{ color: getStatusColor(testResults.crossPlatformCompatibility.crossPlatformSupported ? 'passed' : 'failed') }}>
                    ✅
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Mixed Mode:</span>
                  <span style={{ color: getStatusColor(testResults.crossPlatformCompatibility.mixedModeSupported ? 'passed' : 'failed') }}>
          ...
          <div className="max-h-64 overflow-y-auto space-y-2">
            {logs.map((log) => (
              <div
                ...
                style={{
                  background: log.level === 'error' ? '#2d1b1b' : 
                             log.level === 'warning' ? '#2d2b1b' : 
                             log.level === 'success' ? '#1b2d1b' : '#1b1b2d'
                }}
              >
                ...
                {log.data && (
                  <details className="ml-2">
                    <summary style={{ cursor: 'pointer', fontSize: '12px' }}>Details</summary>
