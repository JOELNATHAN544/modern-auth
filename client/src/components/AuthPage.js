  ...
  const handlePasswordAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        toast.error(
          "Password authentication not implemented in this demo. Please use passkeys.",
        );
      } else {
        // Simulate password registration
        toast.error(
          "Password registration not implemented in this demo. Please use passkeys.",
        );
      }
    } catch (error) {
      ...
      if (isLogin) {
        ...
        if (useMultiModal && capabilities?.platformAuthenticator) {
          ...
          if (result.success) {
            ...
            if (result.usedFallback) {
              toast.success(
                `Logged in using fallback method: ${result.fallbackMethod}`,
              );
            } else {
          ...
          const begin = await axios.post(
            ...
            formData.email ? { username: formData.email } : {},
          );

          // If allowCredentials is empty, suggest cross-device options
          if (
            Array.isArray(begin.data.allowCredentials) &&
            begin.data.allowCredentials.length === 0 &&
            formData.email
          ) {
            toast("No local passkeys found. Use cross-device authentication.", {
              icon: "ℹ️",
            });
            setShowCrossDeviceAuth(true);
            setLoading(false);
        ...
        if (useMultiModal && capabilities?.platformAuthenticator) {
          ...
          if (result.success) {
            ...
            if (result.usedFallback) {
              toast.success(
                `Account created using fallback method: ${result.fallbackMethod}`,
              );
            } else {
      ...
      >
        <div className="card" style={{ maxWidth: "400px", width: "100%" }}>
          <div className="text-center mb-4">
            <Shield
              size={48}
              style={{ margin: "0 auto 16px", color: "#007bff" }}
            />
            <h1
              style={{
                fontSize: "24px",
          ...
          {isLogin && webauthnSupported && (
            ...
            >
              ...
              <p style={{ fontSize: "12px", opacity: 0.8 }}>
                Don't have your passkey on this device? Enter your email to use
                your phone, QR code, or security key.
              </p>
            </div>
          )}
          ...
          {authType === "passkey" && !webauthnSupported && (
            ...
            >
              ...
              >
                ...
                <li>
                  If no platform authenticator, try your phone or a security key
                </li>
              </ul>
            </div>
          )}
